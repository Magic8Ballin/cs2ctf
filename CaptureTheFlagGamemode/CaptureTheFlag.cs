...
public partial class CaptureTheFlag : BasePlugin
{
 ...
 public override string ModuleDescription => "Adds the Capture the Flag game mode to Counter Strike2";
 ...
 public FakeConVar<bool> CtfDebugClient = new("ctf_debug_client", "Enable client console debug logs for CTF (0/1)", false);
 ...
 private int? _worldBombTIndex;
 private int? _worldBombCTIndex;
 private bool _bombPlacementInFlight = false;
 private Timer? _bombEnsureTimer;
 private int _bombRetryAttempts =0;
 ...
 // Small debug helper
 private void DebugToClient(CCSPlayerController? player, string message)
 {
 var msg = $"[CTF][DBG] {message}";
 Console.WriteLine(msg);
 try
 {
 if (player != null && player.IsValid && CtfDebugClient.Value)
 {
 ...
 if (Enabled.Value)
 {
 Server.ExecuteCommand("ctf_enabled0");
 player.PrintToCenter(Localizer["ctf_disabled"]);
 }
 else
 {
 Server.ExecuteCommand("ctf_enabled1");
 ...
 DebugToClient(player, $"ctf_load: exec={absolute}");
 ...
 DebugToClient(player, "ctf_load: exec done; scheduling base apply");
 AddTimer(0.1f, () =>
 {
 ApplyBasesFromConVars();
 DebugToClient(player, $"bases applied: ct={FormatOrNull(_ctFlag.BasePosition)}, t={FormatOrNull(_tFlag.BasePosition)}");
 });
 // After bases are applied, ensure bombs are placed (retry if needed) using the command invoker if possible
 AddTimer(0.3f, () => EnsureBombsWithRetry(player));
 ...
 // Round start -> ensure bombs are placed at flags
 [GameEventHandler]
 public HookResult OnRoundStart(EventRoundStart @event, GameEventInfo info)
 {
 if (!Enabled.Value) return HookResult.Continue;
 EnsureBombsWithRetry();
 ...
 if (cmd.ArgCount <=1)
 {
 player?.PrintToChat("[CTF] Usage: ctf_require_home_flag0|1");
 return;
 }
 ...
 private static string? FormatOrNull(Vector? v)
 => v is null ? "<null>" : string.Create(CultureInfo.InvariantCulture, $"{v.X:F1} {v.Y:F1} {v.Z:F1}");
 ...
 // Bomb helpers and placement -------------------------------------------------

 private void EnsureBombsWithRetry(CCSPlayerController? preferredSpawner = null)
 {
 // stop any previous retry loop
 _bombEnsureTimer?.Kill();
 _bombRetryAttempts =0;

 // Try immediately, otherwise start a short retry loop
 if (!TryEnsureBombsOnce(preferredSpawner))
 {
 _bombEnsureTimer = AddTimer(0.5f, () =>
 {
 if (TryEnsureBombsOnce(preferredSpawner) || _bombRetryAttempts++ >30)
 {
 _bombEnsureTimer?.Kill();
 }
 }, TimerFlags.REPEAT);
 ...
 private bool TryEnsureBombsOnce(CCSPlayerController? preferredSpawner = null)
 {
 if (_bombPlacementInFlight) return true; // already running
 if (_tFlag.BasePosition is null || _ctFlag.BasePosition is null) return false;

 // If we already have tracked bombs, just re-teleport them
 var hadBoth = TryTeleportTrackedBombsToBases();
 if (hadBoth) return true;

 // Need a live spawner (prefer provided one)
 CCSPlayerController? spawner = null;
 if (preferredSpawner is { IsValid: true, PawnIsAlive: true })
 {
 spawner = preferredSpawner;
 }
 else
 {
 spawner = Utilities.GetPlayers()
 .Where(p => p is { IsValid: true, PawnIsAlive: true })
 .OrderByDescending(p => p.Team == CsTeam.CounterTerrorist) // prefer CT
 .FirstOrDefault();
 }
 ...
 // Snapshot existing C4 entity indices
 var before = Utilities.FindAllEntitiesByDesignerName<CBaseEntity>("weapon_c4").Select(e => e?.Index ??0u).ToHashSet();

 _bombPlacementInFlight = true;

 if (spawner.Team == CsTeam.CounterTerrorist)
 {
 // Give two bombs; both will drop to world (CT cannot equip)
 spawner.GiveNamedItem("weapon_c4");
 Server.NextFrame(() =>
 {
 spawner.GiveNamedItem("weapon_c4");
 Server.NextFrame(() => FinalizeBombSpawnAndPlace(spawner, before, removeHeldFromThisPlayer: false));
 });
 }
 else // Terrorist
 {
 // Give and force-drop two bombs without removing items, using slot5 and only dropping when C4 is active
 spawner.GiveNamedItem("weapon_c4");
 Server.NextFrame(() =>
 {
 ForceDropC4(spawner, retries:6, () =>
 {
 spawner.GiveNamedItem("weapon_c4");
 Server.NextFrame(() =>
 {
 ForceDropC4(spawner, retries:6, () =>
 {
 // Allow one more frame for world entities to appear then finalize
 Server.NextFrame(() => FinalizeBombSpawnAndPlace(spawner, before, removeHeldFromThisPlayer: false));
 });
 });
 });
 ...
 return true; // we've scheduled work
 }

 // Repeatedly tries to select slot5 and drop the C4 without affecting other weapons
 private void ForceDropC4(CCSPlayerController player, int retries, Action onDone)
 {
 try
 {
 TryInvokeClientCommand(player, "slot5");
 Server.NextFrame(() =>
 {
 if (PlayerActiveWeaponIsC4(player))
 {
 TryInvokeDropActiveWeapon(player);
 onDone();
 }
 else if (retries >0)
 {
 ForceDropC4(player, retries -1, onDone);
 }
 else
 {
 DebugToClient(player, "Failed to activate C4 after retries; skipping drop to avoid dropping pistol/knife");
 onDone();
 }
 });
 }
 ...
 private void TryInvokeClientCommand(CCSPlayerController player, string cmd)
 {
 try
 {
 var t = player.GetType();
 var m = t.GetMethod("ExecuteClientCommand", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
 ?? t.GetMethod("ClientCommand", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)
 ?? t.GetMethod("FakeClientCommand", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
 if (m != null)
 {
 m.Invoke(player, new object[] { cmd });
 DebugToClient(player, $"invoke client cmd: {cmd}");
 }
 else
 {
 DebugToClient(player, $"no client-cmd method to run: {cmd}");
 }
 }
 catch (Exception ex)
 {
 DebugToClient(player, $"client cmd '{cmd}' failed: {ex.Message}");
 }
 }

 private void TryInvokeDropActiveWeapon(CCSPlayerController player)
 {
 try
 {
 var t = player.GetType();
 var m = t.GetMethod("DropActiveWeapon", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
 if (m != null)
 {
 m.Invoke(player, null);
 DebugToClient(player, "dropactiveweapon invoked");
 }
 else
 {
 DebugToClient(player, "DropActiveWeapon() not available; skipping");
 }
 }
 catch (Exception ex)
 {
 DebugToClient(player, $"dropactiveweapon failed: {ex.Message}");
 }
 }

 private bool PlayerActiveWeaponIsC4(CCSPlayerController player)
 {
 try
 {
 var pawn = player.PlayerPawn?.Value;
 ...
 var wsProp = pawnType.GetProperty("WeaponServices", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
 var ws = wsProp?.GetValue(pawn);
 if (ws == null) return false;

 var activeProp = ws.GetType().GetProperty("ActiveWeapon", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
 var handle = activeProp?.GetValue(ws);
 var valueProp = handle?.GetType().GetProperty("Value", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
 var weaponEnt = valueProp?.GetValue(handle) as CBaseEntity;
 if (weaponEnt == null || !weaponEnt.IsValid) return false;

 var name = weaponEnt.DesignerName ?? weaponEnt.Entity?.Name ?? string.Empty;
 return !string.IsNullOrEmpty(name) && name.Contains("weapon_c4", StringComparison.OrdinalIgnoreCase);
 }
 catch { return false; }
 }

 private void FinalizeBombSpawnAndPlace(CCSPlayerController spawner, HashSet<uint> before, bool removeHeldFromThisPlayer = false)
 {
 try
 {
 // Optionally remove any bomb owned by the spawner (T case cleanup)
 if (removeHeldFromThisPlayer)
 {
 var idxRaw = spawner.PlayerPawn?.Value?.Index;
 int? pawnIndex = idxRaw.HasValue ? unchecked((int)idxRaw.Value) : (int?)null;
 if (pawnIndex.HasValue)
 {
 foreach (var ent in Utilities.FindAllEntitiesByDesignerName<CBaseEntity>("weapon_c4"))
 {
 if (ent is null || !ent.IsValid) continue;
 var ownerIdx = TryGetOwnerEntityIndex(ent);
 if (ownerIdx.HasValue && ownerIdx.Value == pawnIndex.Value)
 {
 ...
 // Filter to bombs that are on ground (unowned)
 var ground = new List<CBaseEntity>();
 foreach (var e in Utilities.FindAllEntitiesByDesignerName<CBaseEntity>("weapon_c4"))
 {
 if (e is null || !e.IsValid) continue;
 var ownerIdx = TryGetOwnerEntityIndex(e);
 if (ownerIdx == null)
 {
 ground.Add(e);
 }
 ...
 if (ground.Count <2)
 {
 Console.WriteLine($"[CTF] Could not find two ground C4s (found {ground.Count}).");
 return;
 }

 // Pick two bombs to use for flags
 var bombT = ground[0];
 var bombCT = ground[1];
 _worldBombTIndex = unchecked((int)bombT.Index);
 _worldBombCTIndex = unchecked((int)bombCT.Index);

 // Teleport to bases
 if (_tFlag.BasePosition is not null)
 TeleportBomb(bombT, _tFlag.BasePosition);
 if (_ctFlag.BasePosition is not null)
 TeleportBomb(bombCT, _ctFlag.BasePosition);

 Console.WriteLine("[CTF] Placed BombT and BombCT at flag bases.");
 }
 catch (Exception ex)
 {
 Console.WriteLine($"[CTF] FinalizeBombSpawnAndPlace error: {ex}");
 }
 finally
 {
 _bombPlacementInFlight = false;
 }
 }

 private bool TryTeleportTrackedBombsToBases()
 {
 if (_worldBombTIndex is null || _worldBombCTIndex is null) return false;
 var bombT = FindBombByIndex(_worldBombTIndex.Value);
 var bombCT = FindBombByIndex(_worldBombCTIndex.Value);
 if (bombT is null || bombCT is null) return false;
 ...
 private static CBaseEntity? FindBombByIndex(int index)
 {
 return Utilities
 .FindAllEntitiesByDesignerName<CBaseEntity>("weapon_c4")
 .FirstOrDefault(e => e is not null && e.IsValid && unchecked((int)e.Index) == index);
 }

 private static int? TryGetOwnerEntityIndex(CBaseEntity ent)
 {
 try
 {
 var t = ent.GetType();
 var ownerProp = t.GetProperty("OwnerEntity", BindingFlags.Public | BindingFlags.Instance) ?? t.GetProperty("Owner", BindingFlags.Public | BindingFlags.Instance);
 if (ownerProp == null) return null;
 var handleObj = ownerProp.GetValue(ent);
 if (handleObj == null) return null;
 var valueProp = handleObj.GetType().GetProperty("Value", BindingFlags.Public | BindingFlags.Instance);
 var ownerEntObj = valueProp?.GetValue(handleObj) as CBaseEntity;
 return ownerEntObj != null ? (int?)unchecked((int)ownerEntObj.Index) : null;
 }
 catch { return null; }
 }

 private static void TeleportBomb(CBaseEntity bomb, Vector dest)
 {
 try
 {
 // small lift to avoid ground sticking
 var lifted = new Vector(dest.X, dest.Y, dest.Z +6f);
 bomb.Teleport(lifted, QAngle.Zero, Vector.Zero);
 }
 catch { }
 }
}